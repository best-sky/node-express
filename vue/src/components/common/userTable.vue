<template>
  <div class="order_warp" ref="warp">
    <div class="form">
      <div>
        <el-input placeholder="按手机号查询" v-model="form.userName" style="width:200px;" clearable></el-input>
        <el-select placeholder="按用户权限查询" clearable v-model="form.power">
          <el-option
        ...
        <button class="add" v-if="userId==1" @click="alertDialog(0)">添加管理员</button>
      </div>
      <table class="tableList_table table-hover table-bordered">
        ...
        <tbody>
          ...
          <tr v-for="(tabledata,index) in tableDatas" :key="index">
            ...
            <td>{{tabledata.sex==1?"男":"女"}}</td>
            <td>{{tabledata.power==1?"超级管理员":"普通管理员"}}</td>
            <td>{{tabledata.cold==1?"正常":"被冻结"}}</td>
            ...
            <td>
              ...
              >编辑</button>
              <button class="cold" v-if="power==1" @click="frost(tabledata.userId)">冻结</button>
              <button
                class="del"
                v-if="power==1||tabledata.userId==userId"
                @click="deletes(tabledata.userId)"
              >删除</button>
            </td>
          </tr>
        </tbody>
...
export default {
  ...
  methods: {
    getUserList() {
      ...
      this.$getData("/index/userlist", data, "post", res => {
        //console.log(res);
        this.tableDatas = res.data.data.userlist;
    ...
    find() {
      ...
      this.$getData("/index/userlist/find", data, "post", res => {
        //console.log(res);
        if (res.data.code == 1) {
          this.pagination = 1;
          this.userName = "";
    ...
    deletes(userId) {
      ...
      this.$confirm("此操作将导致该用户无法登陆, 是否继续?", "提示", {
        ...
        this.$getData("/index/userlist/delete", data, "post", res => {
          if (res.data.code == 1) {
            this.$message({
              showClose: true,
              message: res.data.msg,
    ...
    frost(userId) {
      ...
      this.$confirm("此操作将导致该用户无法登陆, 是否继续?", "提示", {
        ...
        this.$getData("/index/userlist/frost", data, "post", res => {
          if (res.data.code == 1) {
            this.$message({
              showClose: true,
              message: res.data.msg,
